import raw from "raw.macro";
const template = raw("./descriptions/template.md");
const readonly = raw("./descriptions/readonly.md");
const dropdown = raw("./descriptions/dropdown.md");
const subtitle = raw("./descriptions/subtitle.md");
const username = raw("./descriptions/username.md");
const kickoff = raw("./descriptions/kickoff.md");
const userid = raw("./descriptions/userid.md");
const html = raw("./descriptions/html.md");
const innerhtml = raw("./descriptions/innerhtml.md");
const json = raw("./descriptions/json.md");
const url = raw("./descriptions/url.md");
const rmid = raw("./descriptions/rmid.md");
const rmprops = raw("./descriptions/rmprops.md");
const phd = raw("./descriptions/phd.md");
const seo = raw("./descriptions/seo.md");
const seodata = raw("./descriptions/seodata.md");
const xmlhttprequest = raw("./descriptions/xmlhttprequest.md");
const checkbox = raw("./descriptions/checkbox.md");

const descriptions = {
  template,
  readonly,
  dropdown,
  subtitle,
  username,
  kickoff,
  userid,
  html,
  innerhtml,
  json,
  url,
  rmid,
  rmprops,
  phd,
  seo,
  seodata,
  xmlhttprequest,
  checkbox,
};
export default descriptions;
